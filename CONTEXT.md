...
| Término                              | Definición                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Notebook**                         | El contenedor de notas (ex-**Course**, ADR 0014): un curso, un libro, un cuaderno de escritura — lo que venga. La diferencia vive en `area` (texto libre), **no** hay campo `kind`. Tiene `status` (`active` / `paused` / `done`), `started_at`, `finished_at`. No decir "curso" para el contenedor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Note**                             | Una nota dentro de un notebook. `content` es el documento Tiptap. `position` la ordena.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **read_log**                         | Registro histórico de cada repaso: una fila `(note_id, read_at)` por lectura. **Nunca se borra.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Progreso derivado**                | `notas leídas / total del notebook`. No se guarda: sale de `COUNT(*)` sobre `read_log`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **Cola de repaso**                   | **Lo construido hoy:** `derive.reviewQueue(snapshot, 3)` — un batch de **3** notas/flashcards vivas de notebooks vivos, las más viejas primero, `nulls first` (nunca-leídas primero). El `status` del notebook **no** filtra: `active`, `paused` y `done` entran igual. Una nota sin notebook (`notebook_id` null) **no** entra. Devuelve **refs sin `content`**: el cuerpo de la que estás mirando lo pide Repaso con `store.note(id)`. El batch se **congela** en el cliente al montar —marcar leído invalida el snapshot, y sin congelarlo se reordenaría abajo del usuario—; "Cargar más" es lo único que lo descongela. `J`/`K` se mueven adentro del batch sin pegarle al store. Su recorrido y freeze los **ownéa la Sesión de repaso** (`src/review/review-session.ts`). La RPC `review_queue()` sigue en la DB **sin llamador** (ADR 0011). |
| **Sesión de repaso**                 | La máquina que **recorre** la Cola de repaso dentro de la pantalla Repaso. Estado efímero: ítem actual, posición (`2 / 3`), `revealed` (flashcard), `marked` (ya calificada), queue congelada e `index`. Vive en `src/review/review-session.ts` como deep module: pura `createReviewSession` + hook `useReviewSession` que wirea `Snapshot` y `store.save`. Esconde `queue` congelada, `markedIds` (idempotencia), `seen[]` (back-stack futuro para `J`) y `QueueStrategy` interno; expone `item · position · marked · revealed · reads · reveal() · mark(grade?) · next() · prev() · loadMore()`. `dialogOpen`/`confirmingDelete` **no** son sesión: quedan en el render. Cuando se construya la Cola one-by-one, vive **dentro** de la Sesión sin tocar JSX (ADR 0012).                                                                            |
| **Cola one-by-one (NO construido)**  | Diseño deseado, todavía sin implementar: `review_queue(exclude_notebook_id, exclude_note_ids)` sirviendo **una** nota a la vez, con **intercalado forzado** (dos repasos seguidos nunca del mismo `notebook_id` si hay alternativa), `seen[]` en el cliente como back-stack de `J` y `exclude_note_ids`, y fallback soltando el notebook excluido. Estaba escrito acá como si existiera — no existe: la RPC no toma argumentos y `review.tsx` no tiene `seen[]`. Si se construye, hay que hacerlo **en los dos adapters** y **dentro de la Sesión de repaso** vía `QueueStrategy` interno, sin tocar `Review()` (ADR 0012). Ver `.scratch/retention-system/to-grill-retention-system.md`.                                                                                                                                                            |
...
| **Flashcard**                        | Una nota con `kind = 'flashcard'`: el `title` es la pregunta y el `content` la respuesta. **No es tabla propia** (ADR 0010). Se generan con AI desde las notas del notebook; se repasan en Repaso (revelar → autoevaluar) y no aparecen en la lista de notas del notebook.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **% de retención**                   | `correctos / autoevaluaciones` del notebook, derivado de `read_log.grade` — no se guarda (ADR 0003). Ya no se muestra: el sidebar del notebook (rediseño "Bitácora") muestra `% repasado`. `retention()` de `derive.ts` queda disponible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Soft delete**                      | Borrado lógico vía `deleted_at`. La app **nunca** hace `DELETE`. Toda query filtra `deleted_at is null`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **flag `imported`**                  | Marca notas/notebooks migrados de Notion cuyas fechas son estimadas (`created_time` como aprox. de `started_at`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **`days` (hábito)**                  | Los días en que se _planea_ hacerlo (`{1,3,5}`). **Recordatorio, no regla**: no entra en ningún cálculo. Un hábito de días fijos se modela como cupo (`count 3/week`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Cumplimiento**                     | Un hábito `good` cumple al llegar al target (piso); uno `bad`, mientras no lo pase (techo).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Racha de hábito**                  | Períodos consecutivos cumplidos, **estricta**: un período cerrado parcial rompe; sólo el período en curso de un `good` no corta. No confundir con la racha de `read_log`, que son días con repaso.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Serie de hábito**                  | La muestra de celdas que `deriveHabit` deriva, **en el período del hábito** (14 días / 7 semanas / 6 meses, ADR 0013). Alimenta los dots del tile (últimos 7) y el panel de hover. La DB sigue guardando por día (ADR 0009): agregar por período es derivación, no almacenamiento.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Cronómetro (Timer)**               | El contador hacia arriba de los hábitos `time`. Estado efímero en `localStorage` (`{ habitId, startedAt, startedDay }`), **no dato de negocio**: el dato entra en `habit_log` al pausar. Un solo timer global a la vez; dar play en otro hábito pausa el anterior primero. El auto-finish corta al cruzar el `target` del período de `startedDay` (sólo `good`; un `bad` nunca se auto-corta).                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Día de atribución (`startedDay`)** | El día LOCAL en que un Cronómetro **arrancó** (`dayKey(new Date(startedAt))`), congelado al `startTimer`. TODO el `elapsed` se acredita a ese día — pausar 00:19 tras arrancar 23:59 escribe en el día de ayer. Nunca se parte a medianoche, sin cap de 24h. La base del upsert es la fila de `startedDay` y el auto-finish se evalúa contra el período que lo contiene.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
```sql
notebooks(id, user_id, name, status, started_at, finished_at, icon, source, area, deleted_at, created_at)
  -- ex-courses (ADR 0014): un curso, un libro, un cuaderno de escritura — sin campo kind.
...
1. **Repaso** — la que abre 2–3×/día. La cola trae **una** nota/flashcard a la vez, intercalada por `notebook_id` (ver Glosario). Corta al llegar a la meta del día: sale el `Empty` con "Cargar más", que libera la cola por el resto de la sesión (el contador sigue honesto — `4/3`, `5/3`).
...
2. **Notebooks** — lista con estado, progreso derivado, fechas. La pantalla del notebook tiene el % de
...
**Course → Notebook (2026-08-xx).** El contenedor deja de llamarse "curso": un notebook puede
ser un curso, un libro (resumen por sesión de lectura) o un cuaderno de escritura — **todo
repasa igual**, sin comportamiento nuevo ni campo `kind` (la diferencia vive en `area`). Fue
decisión consciente del usuario por `/grill-with-docs`, mismo precedente que settings y hábitos:
no es feature nueva, es vocabulario. Renombre completo de una vez — schema, código, UI, glosario
(ADR 0014). Los términos "curso" anteriores de este archivo refieren a notebooks.
...
Edge Function `generate-flashcards` — eso tumbó el blocker de arquitectura que gateaba _todo_ el
