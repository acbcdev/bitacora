-- sort = 'recientes' pasa de ordenar por últ. repaso (last_read) a ordenar por created_at:
-- las notas más nuevas primero, no las repasadas más recientemente. `last_read` sigue viajando
-- en la fila (columna de la tabla), sólo cambia el ORDER BY. Sin CASE propio: para 'recientes'
-- las tres primeras CASE ya dan null en todas las filas, así que cae directo al tiebreak final
-- `f.created_at desc`, que es exactamente lo que se quiere.
